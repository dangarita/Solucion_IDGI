...
namespace IDGI.CultureResource
{
    ...
    public class ResourceEnumConverter : System.ComponentModel.EnumConverter
    {
        ...
        private string GetValueText(CultureInfo culture, object value)
        {
            ...
            string result = _resourceManager.GetString(resourceName, _CultureInfo);
            if (result == null)
                result = resourceName;
            return result;
        }
        ...
        static public Array GetValues(Type enumType, CultureInfo culture)
        {
            List<string> result = new List<string>();
            TypeConverter converter = TypeDescriptor.GetConverter(enumType);
            foreach (Enum value in Enum.GetValues(enumType))
            {
                result.Add(converter.ConvertToString(null, culture, value));
            }

            return result.ToArray();
        }

        static public Array GetValues(Type enumType)
        {
